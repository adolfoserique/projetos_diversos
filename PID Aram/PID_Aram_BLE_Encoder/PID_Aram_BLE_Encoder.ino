...
#define offset          3                                               // Offset to enter PID mode
...
void loop(){
  ...
  switch (select_mode){
    ...
    case 0:
      ...
      // Checks if the clk and dt bits of the encoder are the same
      if(b_en_clk == HIGH && b_en_dt == LOW){

        // Turn on the encoder (encoder only works when interrupts are enabled)
        attachInterrupt(digitalPinToInterrupt(en_dt), counterclockwise_rot, CHANGE);
        detachInterrupt(digitalPinToInterrupt(en_clk));

      }else if(b_en_clk == LOW && b_en_dt == HIGH){

        attachInterrupt(digitalPinToInterrupt(en_clk), clockwise_rot, CHANGE);
        detachInterrupt(digitalPinToInterrupt(en_dt));

      }else if(b_en_clk == LOW && b_en_dt == LOW){
