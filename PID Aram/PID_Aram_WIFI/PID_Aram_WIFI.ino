...
float digital_temp_select_cal = 0;                                      // Digital value provided by WI-FI
...
volatile int digital_pot = 0;                                           // Digital step provided by WI-FI
volatile unsigned long count_time = 0;                                  // Inicial time counter 
volatile unsigned long count_time_delta = 0;                            // Delta time counter
const char* ssid = "ADOLFO";                                            // SSID for WI-FI
const char* password = "992651818a";                                    // Password for WI-FI

// Bitmap for WI-FI icon 16x16
...
// Bitmap for WI-FI off icon 16x16
...
<body>
  <h1>Temperatura Atual: <span id="temp">)rawliteral" + String(temp_cal) + R"rawliteral(</span> °C</h1>
  <h2>Setpoint Atual: <span id="setpoint">)rawliteral" + String(temp_select_cal) + R"rawliteral(</span> °C</h2>


  <p>
    <button onclick="ajustarSetpoint('up')">+</button>
    <button onclick="ajustarSetpoint('down')">-</button>
  </p>

  <p>
    <button onclick="selecionar()">Selecionar</button>
  </p>
  <form action='/dig_set' method='GET'>
  
    Novo Setpoint: <input type='number' id='novoSetpoint' step='0.1'>
    <button onclick="enviarNovoSetpoint()">Enviar</button>
  ...
  <script>
    ...
    function ajustarSetpoint(acao){
      fetch('/' + acao)
        ...
        .catch(error => {
          console.error("Erro ao ajustar setpoint:", error);
        });
    }

    function enviarNovoSetpoint() {
      const valor = document.getElementById('novoSetpoint').value;
      fetch('/dig_set?value=' + valor)
        .then(() => {
          window.location.href = '/set';
        });
    }
...
String setPage(){
  ...
  String novaPagina = R"rawliteral(
<!DOCTYPE html>
<html>
<head>
  <meta charset='UTF-8'>
...
void setup(){
  ...
  server.on("/", homepage);
  server.on("/up", temp_up);
  server.on("/down", temp_down);
  server.on("/temp_set", temp_set);
  server.on("/set", setpage);
  server.on("/cancel", cancel);
  server.on("/dig_set", temp_digital_set);

  server.on("/get_status", []() {
  temp_cal = thermocouple.readCelsius();
  String json = "{\"temperatura\":\"" + String(temp_cal) + "\",\"setpoint\":\"" + String(temp_select_cal) + "\"}";
...
void temp_select_mode(void){
  ...
  if(WiFi.status() == WL_CONNECTED){
    ...
    display.print((char)247);
    display.print("C");
    display.drawBitmap(112, 0, wifi_off_icon, 16, 16, SSD1306_WHITE);
...
void temp_up(void){
  ...
  if(select_mode == 0 && temp_select_display < temp_max){

    digital_pot++;

  }

  String json = "{\"setpoint\":\"" + String(temp_select_cal) + "\"}";
  server.send(200, "application/json", json);
...
void temp_down(void){
  ...
  if(select_mode == 0 && temp_select_display > temp_min){
...
void temp_set(void){
  ...
  if (server.hasArg("value")){
    ...
    float rx_value = server.arg("value").toFloat();

    if(select_mode == 0 && (rx_value >= temp_min) && (rx_value <= temp_max)){
